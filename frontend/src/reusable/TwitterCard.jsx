/**
 * TwitterCard — reusable component displayed after a successful Twitter/X video info fetch.
 *
 * Uses:
 *  - crud/youtube.crud.js  for all network calls
 *  - services/toastMessages.js  for all user-facing notifications
 *  - reusable/Icons.jsx  for SVG icons (including Twitter/X icon)
 */

import React, { useState, useEffect, useRef } from "react";
import { Download, Play, Eye, User, CheckCircle2, Loader2, ImageIcon, Twitter as TwitterIcon } from "@/reusable/Icons";
...
export default function TwitterCard({ videoInfo }) {
  const [downloading, setDownloading] = useState(false);
  const [downloadComplete, setDownloadComplete] = useState(false);
  const [selectedQuality, setSelectedQuality] = useState(() => {
    // Default to highest quality or Original
      ...
      <div className="relative rounded-3xl overflow-hidden liquid-glass-heavy shadow-[0_20px_60px_rgba(0,0,0,0.6)] border border-white/10">
        ...
        <div className="relative z-10 flex flex-col gap-5 p-5">
          ...
          <div className="relative w-full aspect-video rounded-2xl overflow-hidden shadow-2xl border border-white/10 group/thumb">
            ...
            <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/10 backdrop-blur-md text-xs font-bold text-white shadow-lg border border-white/5">
          ...
          {/* Download Thumbnail directly below the preview */}
          ...
          <div>
            {downloading && stats ? (
              <div className="liquid-glass rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between text-xs font-medium text-white">
                  <span className="flex items-center gap-2">
                    <Loader2 className="w-3.5 h-3.5 animate-spin text-[#1DA1F2]" />{stats.status}
                  </span>
                  <span className="font-mono-tech text-[#1DA1F2]">{stats.percent ? Math.round(stats.percent) : 0}%</span>
                </div>
                <Progress value={stats.percent || 0} className="h-1.5 bg-white/10" indicatorClassName="bg-gradient-to-r from-[#1DA1F2] to-sky-400" />
              ...
              <div className="flex items-center justify-center gap-2 w-full h-11 rounded-xl bg-[#1DA1F2]/10 border border-[#1DA1F2]/30 text-[#1DA1F2] font-medium text-sm">
                <CheckCircle2 className="w-4 h-4" /><span>Download Successful</span>
              </div>
            ) : (
              <button onClick={handleDownload} disabled={downloading}
                className="w-full h-11 rounded-xl font-bold text-sm tracking-tight transition-all duration-300 flex items-center justify-center gap-2 transform hover:-translate-y-0.5 bg-white text-black hover:bg-white/95 shadow-lg border border-white/5">
