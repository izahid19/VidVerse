/**
 * InstagramCard — reusable component displayed after a successful Instagram video info fetch.
 *
 * Uses:
 *  - crud/youtube.crud.js  for all network calls
 *  - services/toastMessages.js  for all user-facing notifications
 *  - reusable/Icons.jsx  for SVG icons (including Instagram icon)
 */

import React, { useState, useEffect, useRef } from "react";
import { Download, Play, Eye, User, CheckCircle2, Loader2, ImageIcon, Instagram as InstagramIcon } from "@/reusable/Icons";
...
export default function InstagramCard({ videoInfo }) {
  const [downloading, setDownloading] = useState(false);
  const [downloadComplete, setDownloadComplete] = useState(false);
  const [selectedQuality, setSelectedQuality] = useState(() => {
    // Default to 'Original' quality for Instagram or the first available quality
  ...
  const showQualities = videoInfo.availableQualities && videoInfo.availableQualities.length > 1;
      ...
      <div className="relative rounded-3xl overflow-hidden liquid-glass-heavy shadow-[0_20px_60px_rgba(0,0,0,0.6)] border border-white/10">
        ...
        <div className="relative z-10 flex flex-col gap-5 p-5">
          ...
          <div className="relative w-full rounded-2xl overflow-hidden shadow-2xl border border-white/10 group/thumb"
            ...
            <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-gradient-to-r from-[#f09433] via-[#e6683c] to-[#c52e8d] text-xs font-bold text-white shadow-lg">
          ...
          {/* Quality selection (only if multiple are available) */}
          {showQualities && (
            <div className="space-y-2">
          ...
          <div>
            {downloading && stats ? (
              ...
              <button onClick={handleDownload} disabled={downloading}
                className="w-full h-11 rounded-xl font-bold text-sm tracking-tight transition-all duration-300 flex items-center justify-center gap-2 transform hover:-translate-y-0.5 bg-gradient-to-r from-[#f09433] via-[#e6683c] to-[#c52e8d] text-white hover:opacity-90 shadow-lg">
